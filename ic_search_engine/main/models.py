...
class integrated_circuit:
    def __init__(self,request_dict):
        ...
        self.pin_count = str(request_dict['pin_count']) if 'pin_count' in keys else "40"
    ...
    def get_data(self,format='dict'):
        dict = {"name":self.name,"shape":self.shape,"manufacturer":self.manufacturer,"pin_count":self.pin_count,"description":self.description,"pinout":self.pinout}
        if format == 'dict':
            return dict
        return jsonify(dict)
